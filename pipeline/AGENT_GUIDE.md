...
5. **Strict Scope & Path Integrity:** Never touch files outside `BOUNDARIES.md` or modify `.env`. All absolute file links and paths must anchor to the current active workspace root (`Doulingo`), never legacy templates (e.g. `boilerplate`).
