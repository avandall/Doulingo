...
|---------|----------|-------|---------|------------|-------------------|
| `TASK-001` | Ingest dữ liệu sách từ `output/extracted/` vào SQLite DB | Phase 1 | P0 | `[ ] TODO` | Nạp YAML chunks vào `data/custom_topics.db` |
| `TASK-002` | Tích hợp RAG Layer (`retrieve_dialogues`) vào `ai_engine.process_turn` | Phase 2 | P0 | `[ ] TODO` | Nối `/api/process_turn` với RAG DB |
| `TASK-003` | Nâng cấp Context-Aware Fallback Engine thay cho Mock Fallback tĩnh | Phase 3 | P0 | `[ ] TODO` | Xử lý khi API rate-limit giữ nguyên ngữ cảnh & Level |
| `TASK-004` | Thống nhất 2 Pipeline (Pipeline A & Pipeline B) | Phase 4 | P1 | `[ ] TODO` | Đồng bộ prompt construction & level rules |
| `TASK-005` | Kiểm thử E2E & Verification toàn bộ luồng hội thoại | Phase 5 | P0 | `[ ] TODO` | Chạy test suite & verify 100% PASS |
...
- [ ] Chạy `insert_turso.py` thành công không có lỗi syntax/schema.
- [ ] Bảng `sample_dialogues` và `content_units` trong `data/custom_topics.db` chứa > 500 bản ghi mới.
- [ ] `retrieve_dialogues()` query được dữ liệu thực tế từ các topic sách mới (vd: `Childhood Memories`, `Enhance Your English...`).
...
#### Acceptance Criteria (Tiêu chí hoàn thành)
- [ ] `ai_engine.process_turn()` gọi `retrieve_dialogues()` trước khi dựng prompt.
- [ ] Prompt gửi tới LLM chứa khối `REFERENCE DIALOGUES FROM BOOKS` đúng band level và topic tag.
- [ ] RAG fallback cascade không bị crash khi topic rỗng.
...
#### Acceptance Criteria (Tiêu chí hoàn thành)
- [ ] Khi user nói câu tiêu cực ("I lost my memory"), fallback phản hồi bằng sự cảm thông chứ không nói "That sounds wonderful!".
- [ ] Fallback không bị bẻ lái sang topic ngẫu nhiên khác (như "Best Friends & Personality").
- [ ] Số từ trong câu fallback tuân thủ range `min_words` và `max_words` của level tương ứng (ví dụ Level 9: 45-85 từ).
...
#### Acceptance Criteria (Tiêu chí hoàn thành)
- [ ] Cả `/api/process_turn` và `/api/voice/process_turn` đều tuân thủ `LEVEL_CONFIGS` 20 cấp độ.
- [ ] Cả 2 pipeline đều sử dụng chung RAG retrieval layer.
...
#### Acceptance Criteria (Tiêu chí hoàn thành)
- [ ] Toàn bộ unit tests & integration tests pass 100%.
- [ ] `python3 pipeline/scripts/verify.py` pass Tier 1 checks.
- [ ] Không có console error hay unhandled exception nào khi gọi API.
